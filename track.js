...
async function loadOrder(id) {
  ...
  try {
    ...
    if (!res.ok) {
      result.innerHTML =
        `<p class="loading">No order found with that number.</p>`;
      return;
    }
    ...
    result.innerHTML =
      `<p class="loading">Could not load order. Is the backend server running?</p>`;
  }
}
```
