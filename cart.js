...
function addToCart(product) {
  ...
  if (existing) {
    ...
    cart.push({ product_id: product.id, name: product.name, price: product.price, image_url: product.image_url, quantity: 1 });
  }
  saveCart(cart);
}
function updateQuantity(productId, delta) {
  let cart = getCart();
  cart = cart.map(i => i.product_id === productId ? { ...i, quantity: i.quantity + delta } : i).filter(i => i.quantity > 0);
  saveCart(cart);
  renderCartDrawer();
}
...
function renderCartDrawer() {
  ...
  document.getElementById('cartTotal').textContent = `Rs. ${cartTotal().toLocaleString()}`;
}
...
function renderPriceBreakdown() {
  ...
  document.getElementById('priceBreakdown').innerHTML = `
    ...
    ${discount > 0 ? `<div class="row discount"><span>Discount (${appliedCoupon.code})</span><span>-Rs. ${discount.toLocaleString()}</span></div>` : ''}
    <div class="row grand-total"><span>Total</span><span>Rs. ${total.toLocaleString()}</span></div>
  `;
}
...
async function applyCoupon() {
  ...
  try {
    ...
    if (data.valid) {
      appliedCoupon = { code: code.toUpperCase(), percent: data.percent };
      feedback.innerHTML = `<div class="coupon-feedback valid">✓ ${data.percent}% discount applied!</div>`;
    } else {
      appliedCoupon = null;
      feedback.innerHTML = `<div class="coupon-feedback invalid">Invalid or expired code.</div>`;
    }
    renderPriceBreakdown();
  } catch (err) {
    feedback.innerHTML = `<div class="coupon-feedback invalid">Could not check code right now.</div>`;
  }
}
...
async function submitOrder(e) {
  ...
  const payment_method = document.querySelector('#paymentMethods input[name="paymentMethod"]:checked').value;
  ...
  try {
    const res = await fetch('/api/orders', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ customer_name, email, address, city, payment_method, coupon_code, items })
    });
    const data = await res.json();
    ...
    document.getElementById('confirmTracking').innerHTML = `
      ${trackingTimelineHTML([
        { key: 'received', label: 'Order received', done: true, current: true },
        { key: 'warehouse', label: 'Warehouse' },
        { key: 'packed', label: 'Packed' },
        { key: 'out_for_delivery', label: 'Out for delivery' },
        { key: 'delivered', label: 'Delivered' },
      ])}
      <a href="track.html?id=${data.order_id}" style="display:block; text-align:center; margin-top:14px; font-size:13.5px; color:var(--link);">Track this order →</a>
    `;
